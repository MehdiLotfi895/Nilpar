...
from main.models import OrderBasket, Product, ColorProduct
from decimal import Decimal

class DatabaseCart:
    """
    مدیریت سبد خرید در دیتابیس با استفاده از مدل موجود OrderBasket
    """
    ...
    def remove(self, product, color):
        OrderBasket.objects.filter(user=self.user, product=product, color=color).delete()

    def update_quantity(self, product, color, quantity):
        return self.add(product, color, quantity, override=True)

    def get_items(self):
        items = []
        basket_items = OrderBasket.objects.filter(user=self.user).select_related('product', 'color')
...
class UnifiedCartService:
    """
    سرویس یکپارچه برای مدیریت سبد خرید در هر دو حالت (سشن و دیتابیس)
    """
    @staticmethod
    ...
    def merge_carts(request):
        ...
        for item in session_items:
            try:
                # تلاش برای اضافه کردن محصول سشن به دیتابیس کاربر
                db_cart.add(item['product'], item['color'], item['quantity'])
            except ValueError:
                pass
                
        # پس از ادغام، سبد خرید سشن را پاک کن
        session_cart.clear()